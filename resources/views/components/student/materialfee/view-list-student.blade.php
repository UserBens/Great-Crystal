...
@section('content')
    <div class="container-fluid">
        <h2 class="text-center display-4">{{ strtoupper($type) }} Student</h2>
        <form class="my-5" action="/admin/payment-grades/create/{{ $type }}">
            <div class="row">
                        ...
                        <tbody>
                            @foreach ($data as $el)
                                <tr id={{ 'index_student_' . $el->id }}>
                                    ...
                                    <td>{{ $el->name }}</td>
                                    <td>{{ $el->grade->name }}</td>
                                    <td>{{ $el->grade->class }}</td>
                                    <td>
                                        @if ($el->material_fee)
                                            Rp {{ number_format($el->material_fee->amount, 0, ',', '.') }}
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td class="text-center">
                                        @if ($el->material_fee)
                                            {{ $el->material_fee->installment ?? 0 }} terms
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td>
                                        @if ($el->material_fee)
                                            Rp {{ number_format($el->material_fee->amount_installment, 0, ',', '.') }}
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td>
                                        @if ($el->material_fee)
                                            {{ ucfirst($el->material_fee->type) }}
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td>
                                        <span class="badge badge-{{ $el->material_fee ? 'success' : 'danger' }}">
                                            {{ $el->material_fee ? 'Already set' : 'Not set yet' }}
                                        </span>
                                    </td>
                                    <td class="project-actions text-center">
                                        <a class="btn btn-primary btn-sm"
                                            href="/admin/payment-grades/detail/{{ $el->unique_id }}/{{ $type }}">
                                            <i class="fas fa-folder"></i>
                                            View
                                        </a>
                                        <a class="btn btn-success btn-sm"
                                            href="{{ route('payment.materialfee.create-form', ['student_id' => $el->unique_id, 'type' => $type]) }}">
