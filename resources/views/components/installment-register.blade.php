                            ...
                            <form method="POST" action={{ route('action.edit.installment', ['bill_id' => $data->id]) }}>
                                ...
                                <div class="card card-dark">
                                    <div class="card-header">
                                        <h3 class="card-title">Edit {{ strtolower($data->type) }} installment for
                                            {{ $data->student->name }}
                                        </h3>
                                    </div>
                                    <!-- /.card-header -->
                                        ...
                                        @if (sizeof($data->bill_installments) > 0)
                                            @foreach ($data->bill_installments as $idx => $item)
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-md-12">
                                                        <label
                                                            for="amount">{{ $item->type . ' intallment ' . $item->subject }}</label>
                                                        <div class="input-group">
                                                            <div class="input-group-prepend">
                                                                <span class="input-group-text">Rp.</span>
                                                            </div>
                                                            <input name="{{ 'index_' . $item->id }}" type="text"
                                                                class="form-control" id="amount{{ $idx }}"
                                                                placeholder="Enter amount capital fee"
                                                                value="{{ old('index_' . $item->id) ? old('index_' . $item->id) : number_format($item->amount_installment, 0, ',', '.') }}">
                                                        </div>
                                                        @if ($errors->any())
                            ...
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        Are you sure want to update installment {{ $data->type }} from
                                        {{ $data->student->name }}?
                                    </div>
                                    ...
                                    @if (sizeof($data->bill_installments) > 0)
                                        @foreach ($data->bill_installments as $idx => $item)
                                            <tr>
                                                ...
                                                <td align="right">Rp.
                                                    {{ number_format((string) $item->amount_installment, 0, ',', '.') }}
                                                </td>
                                            </tr>
                                        @endforeach
                                    @endif
                            ...
                            <table>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td align="right"><b>Rp.
                                                {{ number_format((string) $data->amount - $data->dp, 0, ',', '.') }}</b>
                                        </td>
                                    </tr>
                                    <tr>
                            ...
                            <table>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td align="right">Rp. {{ number_format((string) $data->amount, 0, ',', '.') }}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                                ...
                                <div class="card-body">
                                    @if (sizeof($data->bill_installments) > 0)
                                        @foreach ($data->bill_installments as $idx => $item)
                                            <div class="form-group row">
                                                <div class="col-md-12">
                                                    <label
                                                        for="amount">{{ $item->type . ' installment ' . $item->subject }}</label>
                                                    <div class="input-group">
                                                        <div class="input-group-prepend">
                                                            <span class="input-group-text">Rp.</span>
                                                        </div>
                                    ...
                                    <div class="form-group mt-4">
                                        <div class="icheck-primary">
                                            ...
                                            <label for="sendEmail">I agree with the term.</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            Are you sure you want to update installment {{ $data->type }} for
                                            {{ $data->student->name }}?
                                        </div>
                                        <div class="modal-footer">
                        ...
                        <div class="card mb-4 p-4">
                                    ...
                                    @if (sizeof($data->bill_installments) > 0)
                                        @foreach ($data->bill_installments as $idx => $item)
                                            <tr>
                                                <td align="left">{{ $item->type }} ({{ $idx + 1 }})</td>
                                                <td align="right">Rp.
                                                    {{ number_format($item->amount_installment, 0, ',', '.') }}</td>
                                            </tr>
                                        @endforeach
                                    @endif
                                </tbody>
                            ...
                            <table class="table table-borderless">
                                <tbody>
                                    <tr>
                                        <td align="left"><b>Amount</b></td>
                                        <td align="right"><b>Rp.
                                                {{ number_format($data->amount - $data->dp, 0, ',', '.') }}</b></td>
                                    </tr>
                                    <tr>
                                        <td align="left">Done Payment</td>
                                        <td align="right">Rp. {{ number_format($data->dp, 0, ',', '.') }}</td>
                                    </tr>
                                </tbody>
                            </table>
                            ...
                            <table class="table table-borderless">
                                <tbody>
                                    <tr>
                                        <td align="left">Total</td>
                                        <td align="right">Rp. {{ number_format($data->amount, 0, ',', '.') }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
